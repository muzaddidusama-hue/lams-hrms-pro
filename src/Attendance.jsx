...
export default function Attendance({ user }) {
    ...
    // 🚀 ১০০% সিকিউরড রোল চেকিং (CEO, Admin, Manager)
    ...
    const fetchAllData = async () => {
        ...
        try {
            if (isManagerial) {
                ...
                const { data: geoData } = await supabase.from('company_settings').select('*').eq('id', 1).maybeSingle();
            ...
            if (!isManagerial) reqQuery = reqQuery.eq('emp_id', currentUser?.emp_id);
    ...
    const handleUpdateGeoSettings = async (e) => {
        e.preventDefault();
        await supabase.from('company_settings').upsert({ id: 1, office_lat: geoSettings.lat, office_lng: geoSettings.lng, office_radius: geoSettings.radius });
    ...
    // 👨‍💼 ================= REGULAR EMPLOYEE VIEW (Managerial ছাড়া সবাই এটা দেখবে) =================
    if (!isManagerial) {
        return (
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[300px]">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">
                            <tr><th className="p-6">Date</th><th className="p-6">Check In</th><th className="p-6">Check Out</th><th className="p-6 text-right">Status</th></tr>
                        </thead>
                        ...
                        {requests.length > 0 ? requests.map(req => (
                            <div key={req.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                                <div className="flex justify-between items-start">
                                    <div><h4 className="font-bold text-slate-900">{req.name}</h4><p className="text-[10px] text-slate-400 font-bold uppercase mt-1">For: {req.date}</p></div>
                                ...
                                {req.photo_url && <a href={req.photo_url} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-blue-500 hover:underline"><i className="fa-solid fa-paperclip mr-1"></i>View Attachment</a>}
                                {req.status === 'Pending' && (
                ...
                <div className="space-y-8">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                        ...
                        <div className="bg-slate-950 p-6 rounded-2xl flex flex-col md:flex-row gap-6 md:gap-12 text-white items-center">
                            <div><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Current Latitude</p><p className="font-mono text-lg">{geoSettings.lat || 'Not Set'}</p></div>
                            <div><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Current Longitude</p><p className="font-mono text-lg">{geoSettings.lng || 'Not Set'}</p></div>
                            <div className="flex-1 border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-8"><p className="text-[9px] font-black uppercase tracking-widest text-emerald-400 mb-1">Active Radius</p><p className="text-3xl font-black">{geoSettings.radius ? `${geoSettings.radius} Meters` : 'Not Set'}</p></div>
                        </div>
                        <form onSubmit={handleUpdateGeoSettings} className="space-y-4 pt-4 border-t border-slate-50">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2">Latitude</label><input type="number" step="any" value={geoSettings.lat} onChange={e => setGeoSettings({...geoSettings, lat: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 border-none font-bold text-sm outline-none" required /></div>
                                <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2">Longitude</label><input type="number" step="any" value={geoSettings.lng} onChange={e => setGeoSettings({...geoSettings, lng: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 border-none font-bold text-sm outline-none" required /></div>
                                <div><label className="block text-[10px] font-bold uppercase text-slate-400 mb-2">Radius (Meters)</label><input type="number" value={geoSettings.radius} onChange={e => setGeoSettings({...geoSettings, radius: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 border-none font-bold text-sm outline-none" required /></div>
                            </div>
