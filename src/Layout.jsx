...
export default function Layout({ user, onLogout }) {
    ...
    useEffect(() => {
        fetchPendingCount();
        if (isAdmin) {
            // ইউনিক চ্যানেল নাম 'global-nav'
            const channel = supabase.channel('global-nav')
                .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'leaves' }, (payload) => {
                    fetchPendingCount(); // ব্যাজ আপডেট
                    setToast(`New Request: ${payload.new.name}`); // পপ-আপ
    ...
    return (
        <div className="flex h-screen bg-slate-50 overflow-hidden font-sans relative">
            ...
            {/* 🔔 পপ-আপ নোটিফিকেশন (নিচের ডান কর্নারে) */}
            {toast && (
                <div className="fixed bottom-24 right-8 z-[1000] bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-[slideIn_0.3s_ease-out] border border-white/10 backdrop-blur-md">
            ...
            {/* Sidebar & Header (আগের কোড একই থাকবে) */}
            ...
            <main className="flex-1 overflow-y-auto relative px-6 lg:px-16 pb-20 pt-8">
                <header className="flex justify-end mb-12">
                    <div onClick={() => navigate('/profile')} className="flex items-center gap-4 p-2 pr-5 bg-white rounded-2xl border border-slate-100 shadow-sm cursor-pointer hover:shadow-md">
